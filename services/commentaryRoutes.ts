...
// Re-exportamos el tipo de makeHeaders para no duplicar lógica
...
export async function handleCommentaryRoutes(
  ...
  if (path === "/api/translate-commentary" && req.method === "POST") {
    try {
      ...
      if (!sourceId || !divId || !targetLang) {
        return new Response(
          JSON.stringify({
            error: "Parámetros requeridos: sourceId, divId, targetLang",
          }),
          { status: 400, headers: makeHeaders("no-store") }
        );
      }
  ...
  // No coincidió ninguna ruta de este módulo
  return null;
}
