...
function parseScofieldXML(xmlContent) {
  const entries = [];
...
async function importEntries(entries, sourceId, lang) {
  const BATCH = 100;
  let total = 0;
  let errors = 0;
...
async function main() {
  ...
  console.log(`✅ Fuente lista en la DB: ${source.name} (id: ${source.id})\n`);

  // 2. Procesar ambos archivos
  for (const fileConfig of FILES_TO_PROCESS) {
    const { lang, filename } = fileConfig;
    const filepath = path.join(__dirname, 'data', filename);

    console.log(`\n-------------------------------------------------`);
    console.log(`▶️  PROCESANDO IDIOMA: ${lang.toUpperCase()} | Archivo: ${filename}`);
    console.log(`-------------------------------------------------`);

    if (!fs.existsSync(filepath)) {
      console.error(`❌ Archivo no encontrado: ${filepath}. Saltando...`);
      continue;
    }
    ...
    const entries = parseScofieldXML(xmlContent);
    ...
    if (entries.length > 0) {
      ...
      console.log(`🎉 ¡IMPORTACIÓN DE ${lang.toUpperCase()} COMPLETADA!`);
  ...
  console.log('\n═════════════════════════════════════════════════');
  console.log('🏁 PROCESO GLOBAL FINALIZADO CON ÉXITO');
  console.log('═════════════════════════════════════════════════\n');
}
